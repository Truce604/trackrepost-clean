...
exports.processRepost = functions.https.onRequest((req, res) => {
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      if (!userId || !campaignId) return res.status(400).json({ error: "Missing userId or campaignId" });
      ...
      if (campaign.userId === userId) return res.status(403).json({ error: "Can't repost your own campaign" });

      const earnedCredits = 1 + (liked ? 1 : 0) + (comment ? 2 : 0);
      if (campaign.credits < earnedCredits) return res.status(400).json({ error: "Not enough campaign credits" });
      ...
      if (existingRepost.exists) return res.status(409).json({ error: "Already reposted" });
